...
- **Allocation strategy**: each room is filled with, as close as possible,
  ...
  intentional, not a bug). Critically, a room keeps being filled — moving
  on to further courses/pairs as needed — **until it's actually full or no
  students remain anywhere**, rather than stopping after just one
  course-pair. This means a room is never left with large empty seats
  just because the specific course-pair it started with happened to run
  out early; it keeps pulling in further course-pairs (or same-semester
  courses, as a last resort) to fill up before moving to the next room.
