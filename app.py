...
from engine import (
    parse_rooms_excel,
    parse_students_docx,
    sort_course_students,
    allocate,
    build_room_sheet_docx,
    build_master_excel,
    build_course_wise_room_list_pdfs_by_semester,
)
...
st.caption(
    ...
    "details, and generate one room sheet (Word) per room, a master "
    "allocation report (Excel), and one course-wise room list PDF per "
    "semester (e.g. S3_.., S5_..) that students can check to find their room."
)
...
if generate:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        sort_course_students(courses)  # department asc, then name asc within department
        ...
        pdf_paths = build_course_wise_room_list_pdfs_by_semester(
            college_name=college_name,
            exam_title=exam_title,
            date_str=date_str,
            day_str=day_str,
        ...
        st.session_state["results"] = {
            ...
            "pdf_files": [(p.name, p.read_bytes()) for p in pdf_paths],
...
if results:
    ...
    for i, (fname, fbytes) in enumerate(results["pdf_files"]):
        st.download_button(
            f"⬇️ Download {fname} (room list for students)",
            data=fbytes,
            file_name=fname,
