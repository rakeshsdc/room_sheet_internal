...
st.caption(
    ...
    "allocation report (Excel), and a single course-wise room list (PDF) "
    "that students can check to find their room number."
)
...
if generate:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        pdf_path = out_dir / "Course_Wise_Room_List.pdf"
        build_course_wise_room_list_pdf(
            college_name=college_name,
            exam_title=exam_title,
            date_str=date_str,
...
if results:
    ...
    st.download_button(
        "⬇️ Download course-wise room list for students (.pdf)",
        data=results["pdf_bytes"],
        file_name="Course_Wise_Room_List.pdf",
