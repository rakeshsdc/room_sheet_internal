...
def _normalize_semester(raw: str) -> str:
    """
    Canonicalizes a semester label so trivial formatting differences in the
    source Word doc (case, spacing, punctuation -- "Sem 5", "SEM  5",
    "sem-5", "Semester 5") never get treated as different semesters. This
    is what both allocate() and the PDF-by-semester split key on, so a
    single course accidentally written in a slightly different style no
    longer produces a spurious extra semester (and extra output file).
    """
    m = re.search(r"(\d+)", raw)
    if m:
...
def parse_students_docx(path: str | Path) -> list[Course]:
    ...
    def flush_course(table) -> None:
        ...
        sem = _normalize_semester(sem_match.group(1)) if sem_match else ""
...
def allocate(rooms: list[Room], courses: list[Course]) -> tuple[list[RoomAllocation], int]:
    ...
    A room keeps being filled -- advancing through further courses/pairs as
    needed -- until it is genuinely full or no students remain anywhere, so
    a room is never left with large empty space just because the specific
    course-pair it started with happened to run out early.

    Rooms are FILLED in order of decreasing capacity (so every room with
    capacity > 50 is filled before any room of 50 or under), and whichever
    semester's course still has the MOST students remaining is always
    picked first as that semester's contribution to the current room. Put
    together, this naturally pairs the biggest courses into the biggest
    rooms first, and lets the smaller/tail-end courses settle into the
    smaller rooms afterwards. The returned list is re-sorted back into the
    original room order given in `rooms`, so output files/reports still
    list rooms in the order you provided them in.
    ...
    """
    # Group courses by semester (semester labels are already normalized by
    # parse_students_docx, so formatting quirks in the source doc can't
    # accidentally split one semester into two groups here).
    ...
    def current_course(sem: str) -> Course | None:
        """The course in this semester with the MOST students still
        remaining (ties broken by original document order) -- this is what
        pairs the biggest courses with the biggest (first-filled) rooms."""
        candidates = [c for c in semester_courses[sem] if c.remaining > 0]
        if not candidates:
            return None
        return max(candidates, key=lambda c: c.remaining)

    def semester_remaining(sem: str) -> int:
        return sum(c.remaining for c in semester_courses[sem])

    # Fill rooms biggest-capacity-first (stable sort keeps the original
    # relative order among rooms that share the same capacity).
    fill_order = sorted(rooms, key=lambda r: -r.capacity)

    allocations: list[RoomAllocation] = []

    for room in fill_order:
        if not any(semester_remaining(s) > 0 for s in semester_order):
        ...
        # Total tolerance for this room: how many seats it's still allowed
        # to run over its nominal capacity by, across every orphan-avoidance
        # absorption made while filling it (not per pairing round).
        overshoot_budget = ORPHAN_THRESHOLD

        # Keep pairing/filling rounds going until the room is full (within
        # tolerance) or there genuinely are no students left anywhere --
        # NOT just until one pairing round runs dry. This is what lets a
        # room whose first course-pair finishes early move on to the next
        # available course(s) instead of being left half-empty while other
        # rooms still have students waiting.
        while capacity_left > 0:
            ...
            if len(active) == 1:
                # Only one semester has students left -- no partner semester
                # to pair with, so (as explicitly permitted as a last
                # resort) this room may span more than one course of that
                # same semester.
                sem = active[0]
                need = capacity_left
                took_any = False
                while need > 0:
                    ...
                    if 0 < leftover <= overshoot_budget:
                        overshoot_budget -= leftover
                        take = course.remaining  # finish the course, don't strand 1-2 students
                    chunk = AllocatedChunk(course=course, start_index=course.cursor, count=take)
                    ra.chunks.append(chunk)
                    course.cursor += take
                    need -= take
                    took_any = True
                capacity_left = max(need, 0)
                if not took_any:
                    break  # safety net against an infinite loop
                continue

            # Two or more semesters active: pair up their CURRENT course
            # only (never a second course of the same semester in the same
            # pairing round) and split what's left of the room's capacity
            # between them as evenly as possible.
            current = {s: current_course(s) for s in active if current_course(s) is not None}
            if not current:
                break
            avail = {s: current[s].remaining for s in current}
            share = _distribute_balanced(capacity_left, avail)
            ...
            # at once could push the room past its remaining tolerance. If
            # so, trim back the extra (beyond each side's natural share)
            # from whichever side has the smallest overshoot first.
            extra_used = sum(max(0, take[s] - min(share.get(s, 0), avail[s])) for s in current)
            if extra_used > overshoot_budget:
                over = extra_used - overshoot_budget
                extras = sorted(
                ...
                for s, extra in extras:
                    ...
                    over -= cut
                extra_used = sum(max(0, take[s] - min(share.get(s, 0), avail[s])) for s in current)
            overshoot_budget -= extra_used

            sum_take = sum(take.values())
            if sum_take <= 0:
                break  # nothing more could be taken; avoid an infinite loop
            ...
            capacity_left = max(capacity_left - sum_take, 0)
            # Loop back around: if either course ran out mid-round, the next
            # round will automatically pick up the next course for that
            # semester (or fall into the single-semester branch above) to
            # keep filling this same room before moving on to the next one.

        allocations.append(ra)

    # Allocation was decided in biggest-capacity-first fill order, but for
    # output (room sheets, master Excel, PDFs) it's clearer to list rooms
    # back in the order they were given in the Rooms Excel.
    original_order = {id(r): i for i, r in enumerate(rooms)}
    allocations.sort(key=lambda ra: original_order[id(ra.room)])
