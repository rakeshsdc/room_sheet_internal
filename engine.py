...
def allocate(rooms: list[Room], courses: list[Course]) -> tuple[list[RoomAllocation], int]:
    """
    Fills each room with, as close as possible, an EQUAL number of students
    from each semester present (e.g. Sem 3 and Sem 5), splitting a room's
    capacity evenly between the semesters that still have students left.

    Within a semester, courses are drawn in the order they appear in the
    Word document, moving to the next course of that same semester once the
    current one runs out. Once an entire semester's students are used up
    (globally), later rooms are filled purely from whichever semester(s)
    remain -- which naturally mixes multiple courses of that same semester
    together, since there's no other semester left to pair with.
    ...
    for room in rooms:
        group_remaining = {sem: semester_remaining(sem) for sem in semester_order}
        if all(v == 0 for v in group_remaining.values()):
            break  # every student already allocated; stop, leaving later rooms unused

        seats_per_semester = _distribute_balanced(room.capacity, group_remaining)

        ra = RoomAllocation(room=room)
        for sem in semester_order:
            need = seats_per_semester.get(sem, 0)
            for course in semester_courses[sem]:
                if need <= 0:
                    break
...
def build_course_wise_room_list_pdf(
    ...
    """
    # Work out which room each individual student ended up in.
    room_of: dict[int, dict[int, str]] = {id(course): {} for course in courses}
    for ra in allocations:
        for chunk in ra.chunks:
