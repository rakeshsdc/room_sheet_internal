...
def build_room_sheet_docx(
    ...
    # Keep the last few rows of the final table glued to the totals/signature
    # footer below (and glue the spacer paragraph after it too), so the
    # footer never ends up alone at the top of a new page with no student
    # rows above it for context.
    if last_table is not None:
        rows_to_glue = last_table.rows[-min(3, len(last_table.rows) - 1):]
        for r in rows_to_glue:
            for cell in r.cells:
                _keep_cell_content_together(cell)
    _keep_with_next(d.paragraphs[-1])  # the spacer paragraph just before the footer
    ...
    footer_table = d.add_table(rows=1, cols=3)
    fc = footer_table.rows[0].cells
    _set_cell_text(fc[0], total_str, bold=True)
    _set_cell_text(fc[1], "Present -", bold=True, align=WD_ALIGN_PARAGRAPH.CENTER)
    _set_cell_text(fc[2], "Absent -", bold=True, align=WD_ALIGN_PARAGRAPH.RIGHT)
    ...
    tblPr.append(borders)
    for cell in fc:
        _keep_cell_content_together(cell)  # glue totals row to what follows it

    spacer2 = d.add_paragraph()
    _keep_with_next(spacer2)
    sig = d.add_paragraph()
    sig.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    run = sig.add_run("Signature of Invigilator")
    run.bold = True
